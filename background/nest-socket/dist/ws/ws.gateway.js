    ...
    handleMessage(client, data) {
        ...
        setInterval(() => {
            for (const c of this.server.clients) {
                const clientsName = Array.from(this.server.clients).map((c) => c.id + c?.name);
                c.send(JSON.stringify({ name: c.name, data: '111', clients: clientsName }));
            }
        }, 1000);
        return (0, rxjs_1.from)([1]).pipe((0, rxjs_1.map)((data) => ({ data })));
    }
};
...
__decorate([
    (0, websockets_1.SubscribeMessage)('dd'),
